...
def tf_iou2d(box_xywh_1, box_xywh_2):
    ...
    left_top = torch.max(box1_min, box2_min)
    bottom_right = torch.min(box1_max, box2_max)
    ### get intersection area
    intersection = torch.max(bottom_right - left_top, 0.0)
    intersection_area = intersection[..., 0] * intersection[..., 1]
    ### get union area
    union_area = box1_area + box2_area - intersection_area
    ### get iou
    if union_area==0:
        ...
        iou = torch.div(intersection_area, union_area + 1e-10)
    return iou
...
def tf_giou3d(box_xyzwhd_1, box_xyzwhd_2, input_size):
    ...
    ### get enclose area
    enclose_left_top = torch.min(box1_min, box2_min)
    enclose_bottom_right = torch.max(box1_max, box2_max)
    enclose_section = enclose_bottom_right - enclose_left_top
    enclose_area = enclose_section[..., 0] * enclose_section[..., 1] * enclose_section[..., 2]
    ### get giou
    if union_area==0:
        ...
        giou = iou - torch.div(enclose_area - union_area, enclose_area + 1e-10)
    return giou
